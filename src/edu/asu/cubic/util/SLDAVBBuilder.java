...
import edu.asu.cubic.dimensionality_reduction.SLDAVB;

public class SLDAVBBuilder {
	...
	public static void extractSLDAFeatures(Properties requiredParameters) {
		try{
			...
			int varIters= 20;//Integer.parseInt(tokens[4]);
			double varConverged= 1e-3;//Double.parseDouble(tokens[5]);
			...
			String responsesFolder = baseFolder+"//responses";
			baseFolder = baseFolder+"/"+baseFeature[0]+featureType[0];
			String modelFolderPath= baseFolder;
			String baseModelName= Utilities.capitalizeFirstLetter(response)+"SLDAVB"+"_"+numTopics+"_"+emIters+"_"+alpha+"_"+delta;
			if(!new File(modelFolderPath+"/"+baseModelName+".model").exists()){
				...
				List<Double> trainingLabelsList = Lists.newArrayList();
				...
				for(int set=0; set< trainingSets.length; set++){
					...
					for(int vid=trainingSetVideos[set][0]; vid<=trainingSetVideos[set][1]; vid++){
						...
						String trainLabelsFileName=  responsesFolder+"/"+Utilities.capitalizeFirstLetter(trainingSets[set])+String.format("%s%03d.csv", Utilities.capitalizeFirstLetter(response),vid);
						...
						String[][] tempLabels = Utilities.readCSVFile(trainLabelsFileName, false);
						for(int d=0; d<tempDocuments.length; d++){
							...
							trainingLabelsList.add(Double.parseDouble(tempLabels[d][1]));
						}
				...
				double[] trainingLabels = new double[totalDocs];
				for(int d=0; d<totalDocs; d++){
					trainingLabels[d] = trainingLabelsList.get(d);
				...
				for(int set=0; set< trainingSets.length; set++){
					...
					for(int vid=trainingSetVideos[set][0]; vid<=trainingSetVideos[set][1]; vid++){
						...
						String trainLabelsFileName=  baseFolder+"/"+baseFeature[0]+"Responses/"+Utilities.capitalizeFirstLetter(trainingSets[set])+String.format("%s%03d.csv", Utilities.capitalizeFirstLetter(response),vid);
						String[][] tempAnnotations= Utilities.readCSVFile(trainLabelsFileName, false);
						for(int d=currDocs; d<currDocs+tempDocuments.length; d++){
							trainingLabels[d]= Double.parseDouble(tempAnnotations[d-currDocs][1]);
						}
						currDocs+= tempDocuments.length;
					}
				}*/
				...
				for(int i= 0; i<numTopics; i++)
					for(int j=0; j<V; j++)
						betas[i][j]=delta;
				System.out.println("Training SLDA VB model");
				SLDAVB trainingModel= new SLDAVB(trainingDocuments, trainingLabels, V,numTopics, alphas[0],delta, modelFolderPath, baseModelName,emIters,emConverged, varIters,varConverged,false);
				...
				for(int set=0; set< trainingSets.length; set++){
					for(int vid=trainingSetVideos[set][0]; vid<=trainingSetVideos[set][1]; vid++){
						String trainFeaturesFolder= baseFolder+"/"+Utilities.capitalizeFirstLetter(response)+"SLDAVB"+"_"+numTopics+"_"+emIters+"_"+alpha+"_"+delta;
			...
			if(!requiredParameters.getProperty("testingSets").trim().equals("")){
				...
				SLDAVB trainingModel= (SLDAVB)new ObjectInputStream(new FileInputStream(modelFolderPath+"/"+baseModelName+".model")).readObject();
				SLDAVB testingModel= null;
				for(int set=0; set< testingSets.length; set++){
					for(int vid=testingSetVideos[set][0]; vid<=testingSetVideos[set][1]; vid++){
						String testFeaturesFolder= baseFolder+"/"+Utilities.capitalizeFirstLetter(response)+"SLDAVB"+"_"+numTopics+"_"+emIters+"_"+alpha+"_"+delta;
						...
						if(!new File(testCSVFilePath).exists()){
							...
							testingModel= new SLDAVB(testDocuments,trainingModel, modelFolderPath, baseModelName, 100,1e-4);
							testingModel.infer();
	...
	public static double[] generateSLDAPredictions(String trainingModelFilePath, String testingFilePath) throws Exception
	{
		...
		SLDAVB trainingModel= (SLDAVB)new ObjectInputStream(new FileInputStream(trainingModelFilePath)).readObject();
		double[] coeffs= trainingModel.getCoeffs();

...
public class LDABuilder {
	...
	public static void extractLDAFeatures(Properties requiredParameters) {
		try{
			...
			int gibbsIterations= Integer.parseInt(requiredParameters.getProperty("dimensionReduction").trim().split("_")[2]);
			...
			String baseModelName= baseFeature[0]+featureType[0]+"LDA"+"_"+numTopics;
			if(!new File(modelFolderPath+"/"+baseModelName+"_"+gibbsIterations+".model").exists()){
				...
				LDAGibbs trainingModel= new LDAGibbs(trainingDocuments, V,numTopics, alphas, betas,modelFolderPath, baseModelName);
				trainingModel.configure(gibbsIterations, gibbsIterations-((int)(gibbsIterations*0.2)), 1, 1);
				...
				for(int set=0; set< trainingSets.length; set++){
					for(int vid=trainingSetVideos[set][0]; vid<=trainingSetVideos[set][1]; vid++){
						String trainFeaturesFolder= baseFolder+"/"+baseFeature[0]+featureType[0]+"LDA"+"_"+numTopics+"_"+gibbsIterations;
			...
			if(!requiredParameters.getProperty("testingSets").trim().equals("")){
				...
				LDAGibbs trainingModel= (LDAGibbs)new ObjectInputStream(new FileInputStream(modelFolderPath+"/"+baseModelName+"_"+gibbsIterations+".model")).readObject();
				LDAGibbs testingModel= null;
				for(int set=0; set< testingSets.length; set++){
					for(int vid=testingSetVideos[set][0]; vid<=testingSetVideos[set][1]; vid++){
						String testFeaturesFolder= baseFolder+"/"+baseFeature[0]+featureType[0]+"LDA"+"_"+numTopics+"_"+gibbsIterations;
						...
						if(!new File(testCSVFilePath).exists()){
							...
							testingModel= new LDAGibbs(testDocuments,trainingModel,numTopics);

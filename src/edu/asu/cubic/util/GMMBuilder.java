...
import edu.asu.cubic.dimensionality_reduction.GMM;

public class GMMBuilder {
	...
	public static void extractGMMFeatures(Properties requiredParameters) {
		try{
			...
			String baseModelName= baseFeature[0]+featureType[0]+"GMM"+"_"+numTopics+"_"+emIters;
			if(!new File(modelFolderPath+"/"+baseModelName+".model").exists()){
				...
				GMM trainingModel= new GMM(trainingDocuments, numTopics, emIters, modelFolderPath, baseModelName);
				trainingModel.runEM();
				double[][] topicDistributions= trainingModel.getMixCoeffs();
				//Utilities.printArray(topicDistributions);
				// generate topic distributions for training docs
				currDocs=0;
				for(int set=0; set< trainingSets.length; set++){
					for(int vid=trainingSetVideos[set][0]; vid<=trainingSetVideos[set][1]; vid++){
						String trainFeaturesFolder= baseFolder+"/"+baseFeature[0]+featureType[0]+"GMM"+"_"+numTopics+"_"+emIters;
			...
			if(!requiredParameters.getProperty("testingSets").trim().equals("")){
				...
				GMM trainingModel= (GMM)new ObjectInputStream(new FileInputStream(modelFolderPath+"/"+baseModelName+".model")).readObject();
				GMM testingModel= null;
				for(int set=0; set< testingSets.length; set++){
					for(int vid=testingSetVideos[set][0]; vid<=testingSetVideos[set][1]; vid++){
						String testFeaturesFolder= baseFolder+"/"+baseFeature[0]+featureType[0]+"GMM"+"_"+numTopics+"_"+emIters;
						...
						if(!new File(testCSVFilePath).exists()){
							...
							testingModel= new GMM(testDocuments,trainingModel, modelFolderPath, baseModelName);
							testingModel.infer();
							double[][] topicDistributions= testingModel.getMixCoeffs();
							//Utilities.printArray(topicDistributions);
